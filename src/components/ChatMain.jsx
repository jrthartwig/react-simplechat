      ...
      {messages.map((msg, i) => (
        <div key={msg.id} className={`flex ${msg.user === 'You' ? 'justify-end' : 'justify-start'}`}>
          <div className={`flex items-end gap-3 max-w-2xl w-full ${msg.user === 'You' ? 'flex-row-reverse' : ''}`}>
            ...
            <div className={
              `rounded-xl shadow-sm px-8 py-6 w-full ` +
              (msg.user === 'You'
                ? (darkMode ? 'bg-blue-900 text-blue-100' : 'bg-blue-100')
                : (darkMode ? 'bg-gray-800 text-gray-100' : 'bg-white'))
            }>
              <div className={
                `font-semibold mb-1 ` + (darkMode ? 'text-blue-200' : 'text-blue-700')
              }>{msg.user === 'You' ? 'You' : 'AI (gpt-4o)'}</div>
              ...
              <div className="mt-2 flex gap-2 items-center">
                {/* Only show on AI/model responses */}
                {msg.user !== 'You' && (
                  <>
                    {/* Copy icon (Font Awesome) */}
                    <button
                      aria-label="Copy message"
                      className={`text-xl p-1 rounded hover:bg-gray-100/50 dark:hover:bg-gray-700/60 transition-colors duration-150 ${darkMode ? 'text-white' : 'text-gray-700'}`}
                      tabIndex={0}
                    >
                    ...
                    <button
                      ...
                      className={`text-xl p-1 rounded hover:bg-gray-100/50 dark:hover:bg-gray-700/60 transition-colors duration-150 ${feedback[msg.id]==='up' ? (darkMode ? 'text-blue-400' : 'text-blue-700') : (darkMode ? 'text-white' : 'text-gray-700')}`}
                    ...
                    <button
                      ...
                      className={`text-xl p-1 rounded hover:bg-gray-100/50 dark:hover:bg-gray-700/60 transition-colors duration-150 ${feedback[msg.id]==='down' ? (darkMode ? 'text-blue-400' : 'text-blue-700') : (darkMode ? 'text-white' : 'text-gray-700')}`}
